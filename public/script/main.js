...
class Personnage {
    ...
    seDeplacer(lieu) {
        if (lieu.personnes.includes(this.nom)) {
            ajouterMessage(`${this.nom} est déjà à la ${this.lieu}.`);
        } else {
            lieu.personnes.push(this.nom);
            this.lieu = lieu.nom; // Met à jour la position actuelle du personnage

            ajouterMessage(`${this.nom} est actuellement à la ${lieu.nom}.`);
        }
    }
    prendre(objet){
        if (this.mainDroite.length === 0){
            ...
            ajouterMessage("tu tiens déjà quelque chose dans ta main droite")
        }
    }
    payerArticle(article) {
        if (this.argent >= article.prix) {
            this.argent -= article.prix;
            ajouterMessage(`${this.nom} a payé ${article.prix}€ pour ${article.nom}. Il reste ${this.argent}€.`);
        } else {
            ajouterMessage(`${this.nom} n'a pas assez d'argent pour acheter ${article.nom}.`);
        }
    }
    couper(ingredient, outil){
        ...
        if (this.mainDroite == outil && ingredient.etats == entier){
            ...
            ajouterMessage(`${ingredient} a été coupé`)
        }
    }
}
...
let poêle = {
    ...
    cuire() {
        if (this.contenu.length === 0) {
            ajouterMessage("La poêle est vide. Rien à cuire.");
            return;
        }

        ajouterMessage("Cuisson en cours...");
        setTimeout(() => {
            let melangeCuit = {
            ...
            ajouterMessage(`Le mélange est maintenant ${melangeCuit.etat} !`);
        }, 4000);
    }
};
...
let bol = {
    ...
    melanger(nomDuMelange){
        ...
        ajouterMessage(`le bol contient maintenant ${nomDuMelange}`)
    }
}
...
ajouterMessage(`${Alexandre.nom} a pris un ${epicerie.paniers[0].type}.`);
...
ajouterMessage(`Argent restant d'Alexandre : ${Alexandre.argent}€.`);
...
while (epicerie.paniers[0].contenu.length > 0) {
    ...
    ajouterMessage(`${ingredient.nom} a été déposé dans le bol.`);
    bol.contenu.push(ingredient); // Ajouter l'ingrédient directement dans le bol
}

ajouterMessage("Tous les ingrédients ont été transférés dans le bol.");
...
ajouterMessage("Le panier a été dépose")

// Retournez à la maison pour continuer l'omelette

Alexandre.seDeplacer(maison);
ajouterMessage("Alexandre est revenu à la maison pour continuer l'omelette.");
...
bol.contenu.forEach((ingredient) => {
    if (ingredient.etats.includes("entier")) {
        ...
        ajouterMessage(`${ingredient.nom} a été coupé.`);
    } else {
        ajouterMessage(`${ingredient.nom} n'a pas besoin d'être coupé.`);
    }
});
